...
export const ChangePassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setUpdating(true);
      const {data :{msg,token}} = await axios.post("/auth/change-password", {
      ...
      showError(error?.response?.data?.msg || "Error ocuured in Changing the password")
    }
    finally {
      setUpdating(false); 
      handleReset();
  ...
  return (
    <> 
  <button className="btn btn-primary rounded-pill ms-2 mt-3" onClick={()=>navigate('/staff')}>🏠 Back to your page</button>
